...
const FloatingChat = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: "Hey! I'm Pal, your AI study assistant. How can I help you today?",
      sender: 'pal',
      timestamp: new Date()
    }
  ]);
  ...
  const getPalResponse = (userMessage: string): string => {
    ...
    if (lowerMessage.includes('math') || lowerMessage.includes('maths')) {
      return "I'd be happy to help with maths! Are you working on algebra, geometry, statistics, or another topic?";
    } else if (lowerMessage.includes('science')) {
      return "Science is fascinating! Which area are you studying - biology, chemistry, or physics?";
    } else if (lowerMessage.includes('help') || lowerMessage.includes('stuck')) {
      return "I'm here to help! What specific topic or problem are you struggling with?";
    } else {
      return "That's a great question! Could you tell me which subject and specific topic you're working on?";
    }
  };
      ...
      >
            ...
            >
              {messages.map((msg, index) => {
                ...
                return (
                  ...
                  >
                    <div className="relative inline-block max-w-[320px] px-5 py-3 text-sm leading-relaxed font-medium">
                      <div
              ...
              {isPalTyping && (
                ...
                >
                  <div className="relative inline-block px-5 py-3 rounded-3xl">
                    <div
                      className="absolute inset-0 bg-gradient-to-br from-white/25 via-white/20 to-white/15 backdrop-blur-xl border border-white/40 rounded-3xl"
                      style={{
                        backdropFilter: 'blur(20px) saturate(150%)',
                        WebkitBackdropFilter: 'blur(20px) saturate(150%)',
                      }}
        ...
        {isExpanded && (
          <div className="absolute bottom-full mb-4 right-2 flex gap-2">
            <Button
              size="sm"
              className="bg-white/15 hover:bg-white/25 text-foreground/70 hover:text-foreground p-2.5 h-7 w-7 rounded-full border border-white/25 backdrop-blur-sm"
        ...
        >
          ...
          <Button
            ...
            className={`relative z-10 transition-all duration-300 ${
              isTyping
                ? 'bg-gradient-to-r from-orange-500 to-pink-500 hover:from-orange-600 hover:to-pink-600 text-white p-2 h-8 w-8 rounded-full shadow-lg'
                : 'bg-white/10 hover:bg-white/20 text-foreground/80 hover:text-foreground p-2 h-8 w-8 rounded-full border border-white/20 backdrop-blur-sm'
            }`}
